...
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@space-scavenger-hunt/ui/components/card";
import { useQuery } from "@tanstack/react-query";
import { motion } from "framer-motion";
import { AlertCircle, Compass, Orbit, Rocket, Shield, Sparkles, Star, Terminal, Trophy, Users } from "lucide-react";
    ...
    <div className="relative min-h-full w-full bg-slate-950 text-slate-100 overflow-y-auto overflow-x-hidden flex flex-col justify-between">
      ...
      <div className="relative z-10 flex-1 flex flex-col max-w-6xl mx-auto w-full px-6 py-12 md:py-20 justify-center items-center gap-16">
        ...
        <div className="text-center space-y-6 max-w-3xl">
          ...
          {statusConfig && (
            ...
            >
              <span className={`h-1.5 w-1.5 rounded-full animate-ping ${statusConfig.indicator}`} />
              <span className={`h-1.5 w-1.5 rounded-full absolute ${statusConfig.indicator}`} />
              {statusConfig.label}
            </motion.div>
          )}
          ...
          >
            The Starfarer-9 has suffered a critical hull failure. Its crew is stranded across orbital coordinates. Enlist with your team, decrypt sector logs, and rescue them before life support systems fail.
          </motion.p>
        </div>
          ...
          <Card className="border border-cyan-500/20 bg-slate-900/60 backdrop-blur-md shadow-[0_0_40px_rgba(6,182,212,0.08)] rounded-none relative overflow-hidden group">
            ...
            <CardHeader className="text-center pb-2">
              ...
              <CardDescription className="text-slate-400 text-xs">
                {session ? "AUTHENTICATED PLAYER PORTAL" : "UNENLISTED PILOT LINK"}
              </CardDescription>
            </CardHeader>
                ...
                <div className="space-y-4">
                  <div className="bg-slate-950/50 p-4 border border-slate-800 rounded-none space-y-3 font-mono text-xs text-left">
                    <div className="flex justify-between border-b border-slate-900 pb-1.5">
                      ...
                      <span className="text-slate-200 font-bold">{me.data?.user.name || session.user.name}</span>
                    </div>
                    <div className="flex justify-between border-b border-slate-900 pb-1.5">
                      <span className="text-slate-500">SECTOR ASSIGNMENT:</span>
                      <span className="text-slate-200">
                        {me.data?.user.role === "ADMIN" ? (
                          <span className="text-purple-400 font-bold">MISSION CONTROL ADMIN</span>
                        ) : me.data?.player?.team ? (
                          <span className="font-bold" style={{ color: me.data.player.team.color || '#38bdf8' }}>
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-slate-200 uppercase">{activityStatus}</span>
                    </div>
                  </div>
        ...
        <div className="w-full space-y-6">
          <div className="flex items-center gap-3 border-b border-slate-800 pb-3">
            <Orbit className="size-5 text-indigo-400 animate-spin" style={{ animationDuration: '10s' }} />
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            >
              ...
              <p className="text-xs text-slate-400 leading-relaxed">
                Form search parties under authorized flight names. Work together in close radio telemetry to share coordinate intelligence.
              </p>
            </motion.div>
            ...
            >
              ...
              <p className="text-xs text-slate-400 leading-relaxed">
                Decrypt distress beacons scattered across the sector. Bypass local mainframes by resolving mathematical & logical sector questions.
              </p>
            </motion.div>
      ...
      <footer className="border-t border-slate-900/80 py-6 px-6 bg-slate-950/80 backdrop-blur-sm relative z-10">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4 text-[10px] font-mono text-slate-500">
          <div>
            SYSTEM: ALPHA-OMEGA SECTOR SCAVENGER // LIFE SUPPORT ACTIVE
          </div>
          <div>
            &copy; {new Date().getFullYear()} DEEP SPACE EXTRACTOR. ALL LOGS SECURED.
          </div>
        </div>
      </footer>
